...
class Application_Model_Parola
{
    ...
    private function interroga(){
       //prendo in entrata solo parole del segmento che mi conta almeno 8 sillabe
        ...
        if (($n = Application_Model_Start::inStringa($value, "(=")) >-1){ //posizione (==++-)
            $stringa = substr($value, $n+3, -1);
            $tmp = explode ("|", $stringa);
            $this->marcata = $tmp[0];
            error_log ("BBBBB " . $this->marcata . "  " . $stringa);
            if (count($tmp)>1){
                $this->marca_sillabe = $tmp[1];
            }
    ...
    //controlla se è perfetto
       ...
       //// cerco la parola nel vocabolario (solo per controllo quantità)
           ...
           // error_log("che parola sto cercando? " . $cerca);
            ...
            $risultato = $this->vocabolario->get($cerca); //$risultato è un array di array
           if ($risultato){
            ...
            //controllo di casi con maiuscola: ne prendiamo nota, ma li mettiamo in subordine
            $pos= array(); //posizione
            ...
            //////////////////////////////////Riportare sul log///////////////////////
                ...
                for ($i=0; $i<count($risultato); $i++){
                   if (ctype_upper(mb_substr($risultato[$i][0], 0, 1))){//controlla se la prima lettera di una parola è maiuscola
                       $pos[] = $i;
                   }
...
//la variabile $this_singulae è quella utilizzata per il log nella pagina SINGULARITER
            if (count($risultato) > 1){
                ...
                for($i=0; $i<count($risultato); $i++){
                    if ($i>0) $this->log .= ", ";
                    $this->log .=  $risultato[$i][0]. "(" . $risultato[$i][2] . ")";
            ...
            ///divisione sillaba dal vocabolario (e da marcata?)
            if ($this->trovato==true){
                    if ($this->taggata==true){
                        foreach ($copia_result as $copia){
                            if (strlen($copia[2])==strlen($this->marcata)){
                                $this->lemma=$copia[0];
                                error_log("TROVATA&TAGGATA " . $this->lemma);
                            }
                        }
                        
                    }

                    error_log("dal vocabolario " . $this->lemma  . " * " . $this->quantitavoc );
                    error_log($item[0] . " @ " . $item[1] . " @ ". $item[2]);

                    $value = $this->lemma ;
                    error_log($value . " passaaaaa????");
                    if (!Application_Model_Start::inStringa ($this->lemma, "v")){
                        $value = Application_Model_Start::cambiareU_V($value);
                        $value = Application_Model_Start::cambiaV_U($value);
                    }
                    $tmp = $this->vocabolario->semivocali($value);
            ...
            //controlla se è perfetto
                    ...
                    $value = str_replace ("jj", "JI", $value);
                    $value = str_replace ("qu", "QU", $value);
                    $value = str_replace ("gu", "GU", $value);
//                    $value = str_replace("su", "SU", $value);
                    $value = str_replace ("II", "JI", $value);
//                    $value = str_replace ("J", "I", $value);
                    $value = str_replace ("QU", "QV", $value);
                    $value = str_replace ("āE", "AE", $value);
                    $value = str_replace ("ăE", "AE", $value);
                    $value = str_replace("ōE", "D", $value);
                    $value = str_replace("ŏE", "D", $value);
                    $value = str_replace("āU", "D", $value);
                    $value = str_replace("ăU", "D", $value);
//                    $value = str_replace("suă", "SVă", $value);
//                    $value = str_replace("suă", "SVă", $value);
//                    $value = str_replace("suā", "SVā", $value);
//                    $value = str_replace("suē", "SVē", $value);
//                    $value = str_replace("SUă", "SVă", $value);
//                    $value = str_replace("SUē", "SVē", $value);
                    
                    

//                    $value = str_replace ("GU", "GV", $value);
//                    $value = str_replace("SU", "SV", $value);
        ...
        //                    $char = mb_split('//', $value, -1);
                            $char = array();
                            for ($i=0; $i<mb_strlen($value); $i++){
                                $char[$i] = mb_substr($value, $i, 1);
                            }
                            $stringa = "";
                            $this->sillabe = array();
                                    ...
                                    if ($j === count($char)-1){
                                        ...
                                        if($vocale === false){
                                            ...
                                            if (Application_Model_Start::is_Vocale($char[$j+1])){
                                                $chiudi = true;
        //                                        if (Application_Model_Start::is_Lunga($char[$j+1])){
...
/// assegno la quantita 'alternativa' del vocabolario alle sillabe
        if ($this->marcata!=""){
            ...
            if (count($this->sillabe)==strlen($this->quantitavoc)){
                ...
                error_log("!!! " . count($this->sillabe). " è il numero di oggetti sillaba disponibili!!!!!!");
            }
